...
const SCOPE_PERMISSIONS = {
    "read-only": new Set([]),
    "risk-actions": new Set(["ESCALATE", "NOTIFY", "PROTECT_POSITION"]),
    full: new Set([
        "ESCALATE",
        "PROTECT_POSITION",
        "NOTIFY",
 ...
 */
export function isAuthorized(agentId, scope, actionType) {
    ...
    console.info(`[agent-guard] ${authorized ? "ALLOW" : "DENY"} | agent=${agentId} action=${actionType} scope=${scope}`);
    return decision;
}
