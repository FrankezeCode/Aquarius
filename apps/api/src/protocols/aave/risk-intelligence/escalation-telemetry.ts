...
export class AccumulatorRingBuffer {
  ...
  record(timestamp: number, accumulator: number): void {
    ...
    if (this.count < RING_BUFFER_MAX) this.count++;
  }
   ...
   */
  getVelocity(windowMs: number = 60_000): number {
  ...
  private getEntry(index: number): Snapshot | null {
 ...
 */
export function deriveStability(velocity: number): StageStability {
  const absVelocity = Math.abs(velocity);
  if (absVelocity < STABLE_THRESHOLD) return "stable";
  if (absVelocity < TRANSITIONING_THRESHOLD) return "transitioning";
  return "escalating";
}
...
export class EscalationTimeline {
  private events: EscalationTimelineEvent[] = [];
